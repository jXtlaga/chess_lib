...
void play_castling(U64 *king_sq1, U64 *rook_sq1, COLOR side, bool is_short_castling) {
    if (side == WHITE) {
        *king_sq1 = is_short_castling ? 0x2 : 0x20;
        *rook_sq1 = is_short_castling ? 0x4 : 0x10;
    } else {
        *king_sq1 = is_short_castling ? 0x0200000000000000 : 0x2000000000000000;
        *rook_sq1 = is_short_castling ? 0x0400000000000000 : 0x1000000000000000;
    }
}
...
void move_and_remove_piece(U64 *current_piece_occ, Pieces_position *enemy_pieces, U64 from_sq1, U64 to){
    ...
    *current_piece_occ |= to;
    remove_enemy_piece(from_sq1, enemy_pieces);
}
Position make_move(Position position, U8 from, U64 to, TYPE_MOVE type_move) {
    ...
    U8 *current_castling_rights = (side == WHITE) ? &new_position.castling_white_rights : &new_position.castling_black_rights;
    ...
    switch (type_move) {
        ...
        case PROMOTION_QUEEN: {
            current_pieces->pawn ^= from_sq1;
            current_pieces->queen |= to;
            break;
        }
        case PROMOTION_ROOK: {
            current_pieces->pawn ^= from_sq1;
            current_pieces->rook |= to;
            break;
        }
        case PROMOTION_BISHOP: {
            current_pieces->pawn ^= from_sq1;
            current_pieces->bishop |= to;
            break;
        }
        case PROMOTION_KNIGHT: {
            current_pieces->pawn ^= from_sq1;
            current_pieces->knight |= to;
            break;
        }
        case ROOK_MOVE: {
            if (side == WHITE) {
